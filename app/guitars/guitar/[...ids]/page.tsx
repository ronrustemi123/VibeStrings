...
export default async function GuitarPage({params, searchParams}: {params: {ids: string[]}; searchParams: {startIndex: number}}) {

    const [brandId, modelId] = params.ids
    const {startIndex} = searchParams

    const {data, error} = await getData({query: findUniqueModel, variables: {brandId: brandId, modelId: modelId}});
        ...
        <>
            <section className={'w-page-width mx-auto  min-h-[60vh] relative'}>
                <div className={'mt-8 flex flex-col relative z-10 gap-2'}>
                    <Link href={`/guitars/${brandId}`} className={'font-medium text-[#3D3D46] flex items-center gap-2 hover:underline'}>
                ...
                <span className={'absolute right-0 -top-16 hidden lg:block z-0 '}>
                    ...
                    <span className={'absolute top-2/6 right-1/7 -rotate-45'}>
                        <Image src={data.findUniqueModel?.image} alt={'model image'} width={400} height={220} className={'object-cover object-center'} />
                    </span>
            </span>
            </section>
            <GuitarPageTabs brandId={brandId} startIndex={startIndex} modelId={modelId} description={data.findUniqueModel?.description} specs={data.findUniqueModel?.specs} musicians={data.findUniqueModel?.musicians} />
        </>
    )
}